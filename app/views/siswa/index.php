<div class="space-y-6" 
     x-data="{ showModal: false, importModalOpen: false, editModalOpen: false, deleteModalOpen: false, detailModalOpen: false, ultahModalOpen: false, qrModalOpen: false, qrToken: '', qrName: '', deleteUrl: '', currentSiswa: {} }"
            ...
            <tbody class="divide-y divide-slate-100">
                ...
                <tr class="hover:bg-slate-50 transition-colors">
                    ...
                    <td class="px-6 py-4 flex justify-center gap-2">
                        <button type="button" @click="openQrModal('<?= htmlspecialchars($s['qr_token']); ?>', '<?= htmlspecialchars($s['nama_lengkap'], ENT_QUOTES); ?>')" class="p-2 text-slate-400 hover:text-purple-600 hover:bg-purple-50 rounded-lg transition-colors" title="Lihat QR Code">
    ...
    <!-- Modal QR Code -->
    <div x-show="qrModalOpen" class="fixed inset-0 z-50 overflow-y-auto" style="display: none;">
        <div x-show="qrModalOpen" x-transition.opacity class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm transition-opacity" @click="qrModalOpen = false"></div>
        <div class="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
            <div x-show="qrModalOpen" x-transition class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-md border border-slate-200">
                <div class="bg-white px-4 pb-4 pt-5 sm:p-6 sm:pb-4 border-b border-slate-100 flex justify-between items-center">
                    <h3 class="text-lg font-semibold leading-6 text-slate-900 flex items-center gap-2">
                        <i class="fas fa-qrcode text-purple-500"></i>
                        QR Code Absensi
                    </h3>
                    <button type="button" @click="qrModalOpen = false" class="text-slate-400 hover:text-slate-500 transition-colors">
                        <i class="fas fa-times text-xl"></i>
                    </button>
                </div>
                <div class="bg-slate-50 px-6 py-8 text-center flex flex-col items-center justify-center">
                    <p class="font-bold text-lg text-slate-800 mb-4" x-text="qrName"></p>
                    <div class="bg-white p-4 rounded-xl shadow-sm border border-slate-200 inline-block mb-4" id="qrContainer">
                        <!-- QR Image generated by JS -->
                        <template x-if="qrToken">
                            <img :src="'https://api.qrserver.com/v1/create-qr-code/?size=200x200&data=' + qrToken" alt="QR Code" class="mx-auto">
                        </template>
                    </div>
                    <p class="text-xs text-slate-500 max-w-xs mx-auto">Gunakan QR Code ini untuk melakukan presensi absensi melalui mesin scanner.</p>
                </div>
                <div class="bg-white px-4 py-4 sm:flex sm:flex-row-reverse sm:px-6 border-t border-slate-200 gap-3">
                    <button type="button" @click="qrModalOpen = false" class="inline-flex w-full justify-center rounded-lg bg-blue-600 px-5 py-2.5 text-sm font-semibold text-white shadow-sm hover:bg-blue-700 sm:w-auto transition-colors">Selesai</button>
                    <a :href="'https://api.qrserver.com/v1/create-qr-code/?size=500x500&data=' + qrToken" download="QRCode.png" target="_blank" class="mt-3 inline-flex w-full justify-center rounded-lg bg-white px-5 py-2.5 text-sm font-semibold text-slate-700 shadow-sm ring-1 ring-inset ring-slate-300 hover:bg-slate-50 sm:mt-0 sm:w-auto transition-colors">Unduh QR</a>
                </div>
            </div>
        </div>
    </div>
...
function openQrModal(token, nama) {
    if(!token) {
