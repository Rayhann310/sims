<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8" x-data="{ modalOpen: false }">
    <div class="flex justify-between items-center mb-6">
        <div>
            ...
            <p class="text-sm text-slate-500 mt-1">Kelola data referensi Master Jurusan / Program studi.</p>
        </div>
            ...
            <table class="min-w-full divide-y divide-slate-200">
                <thead class="bg-slate-50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Nama Jurusan / Program</th>
                        <th scope="col" class="px-6 py-4 text-right text-xs font-semibold text-slate-500 uppercase tracking-wider">Aksi</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-200 bg-white">
                    <?php $no = 1; foreach($data['jurusan'] as $j) : ?>
                    <tr class="hover:bg-slate-50 transition-colors">
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-slate-500"><?= $no++; ?></td>
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="text-sm font-bold text-slate-900"><?= $j['nama_jurusan']; ?></div>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-2">
                            <button type="button" @click="$dispatch('edit-jurusan', <?= htmlspecialchars(json_encode($j)) ?>)" class="text-blue-600 hover:text-blue-900" title="Edit">Edit</button>
                            <a href="<?= BASEURL; ?>/akademik/hapusJurusan/<?= $j['id']; ?>" class="text-red-600 hover:text-red-900" onclick="return confirm('Yakin ingin menghapus jurusan ini?');" title="Hapus">Hapus</a>
                        </td>
                    </tr>
                    <?php endforeach; ?>
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div x-show="modalOpen" x-transition class="relative inline-block w-full max-w-md p-6 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl sm:my-8">
                <div class="flex items-center justify-between mb-5">
                    <h3 class="text-lg font-bold text-slate-900">Tambah Master Jurusan</h3>
                    <button @click="modalOpen = false" class="text-slate-400 hover:text-slate-500 transition-colors">
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                    </button>
                </div>
                
                <form action="<?= BASEURL; ?>/akademik/tambahJurusan" method="post">
                    <div class="space-y-4">
                        <div>
                            <label for="nama_jurusan" class="block text-sm font-medium text-slate-700 mb-1">Nama Jurusan / Program</label>
                            <input type="text" name="nama_jurusan" id="nama_jurusan" placeholder="Contoh: MIPA, IPS, atau UMUM" required class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all">
                        </div>
                    </div>
    ...
    <!-- Modal Edit -->
    <div x-data="{ modalEditOpen: false, formData: {id: '', nama_jurusan: ''} }" 
         @edit-jurusan.window="modalEditOpen = true; formData = $event.detail;" 
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:p-0">
            ...
            <div x-show="modalEditOpen" x-transition class="relative inline-block w-full max-w-md p-6 overflow-hidden text-left align-middle transition-all transform bg-white shadow-xl rounded-2xl sm:my-8">
                <div class="flex items-center justify-between mb-5">
                    <h3 class="text-lg font-bold text-slate-900">Edit Master Jurusan</h3>
                    <button @click="modalEditOpen = false" class="text-slate-400 hover:text-slate-500 transition-colors">
                        <svg class="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path></svg>
                    </button>
                </div>
                
                <form action="<?= BASEURL; ?>/akademik/editJurusan" method="post">
                    <input type="hidden" name="id" x-model="formData.id">
                    <div class="space-y-4">
                        <div>
                            <label class="block text-sm font-medium text-slate-700 mb-1">Nama Jurusan / Program</label>
                            <input type="text" name="nama_jurusan" x-model="formData.nama_jurusan" required class="w-full px-4 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none transition-all">
                        </div>
                    </div>
