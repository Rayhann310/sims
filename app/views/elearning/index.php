<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <div class="mb-8 text-center sm:text-left">
        ...
        <p class="text-slate-500 mt-2 text-lg">Akses materi dan tugas untuk setiap mata pelajaran Anda.</p>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        <?php if(empty($data['jadwal'])): ?>
            <div class="col-span-full py-12 text-center text-slate-500 bg-white rounded-2xl shadow-sm border border-slate-200">
                ...
                <p class="text-lg font-medium text-slate-900">Belum Ada Kelas</p>
                <p class="mt-1">Anda belum terdaftar dalam jadwal mata pelajaran apapun.</p>
            </div>
        <?php endif; ?>

        <?php foreach($data['jadwal'] as $j): ?>
        <a href="<?= BASEURL; ?>/elearning/detail/<?= $j['id']; ?>" class="group block bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden hover:shadow-xl hover:border-blue-300 transition-all duration-300 transform hover:-translate-y-1">
            ...
            <div class="p-6 relative">
                <div class="w-12 h-12 bg-white rounded-xl shadow-md border border-slate-100 flex items-center justify-center absolute -top-6 left-6 text-blue-600">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6.253v13m0-13C10.832 5.477 9.246 5 7.5 5S4.168 5.477 3 6.253v13C4.168 18.477 5.754 18 7.5 18s3.332.477 4.5 1.253m0-13C13.168 5.477 14.754 5 16.5 5c1.747 0 3.332.477 4.5 1.253v13C19.832 18.477 18.247 18 16.5 18c-1.746 0-3.332.477-4.5 1.253"></path></svg>
                </div>
                <div class="mt-4">
                    <h3 class="text-xl font-bold text-slate-900 group-hover:text-blue-600 transition-colors"><?= $j['nama_mapel']; ?></h3>
                    <p class="text-sm font-medium text-slate-500 mt-1"><?= isset($j['nama_guru']) ? $j['nama_guru'] : 'Rombel: '.$j['nama_rombel']; ?></p>
                </div>
                <div class="mt-6 flex items-center justify-between text-sm text-slate-500 border-t border-slate-100 pt-4">
                    ...
                    <span class="text-blue-600 font-medium group-hover:underline">Masuk Kelas &rarr;</span>
                </div>
            </div>
        </a>
        <?php endforeach; ?>
