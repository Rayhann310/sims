<div class="p-6">
    ...
    <div class="mb-6 flex items-center justify-between">
        <div>
            ...
            <p class="text-sm text-slate-500 mt-1">Perbarui konten artikel atau berita yang sudah ada.</p>
        </div>
    ...
    <form action="<?= BASEURL; ?>/artikel/update" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
                <div>
                    ...
                    <input type="text" name="judul" required value="<?= htmlspecialchars($data['artikel']['judul']); ?>" placeholder="Tuliskan judul artikel..." class="w-full px-4 py-2.5 text-base border border-slate-300 rounded-lg focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500">
                </div>

                <div>
                    <label class="block text-sm font-semibold text-slate-700 mb-1">Ringkasan Singkat</label>
                    <textarea name="ringkasan" rows="2" placeholder="Ringkasan singkat artikel..." class="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-emerald-500 focus:border-emerald-500"><?= htmlspecialchars($data['artikel']['ringkasan']); ?></textarea>
                </div>
                ...
                <div>
                    ...
                    <textarea id="editorIsi" name="isi" rows="16" required placeholder="Isi artikel..." class="w-full p-4 text-sm font-mono border border-slate-300 rounded-b-lg focus:ring-emerald-500 focus:border-emerald-500 leading-relaxed"><?= htmlspecialchars($data['artikel']['isi']); ?></textarea>
                </div>
            </div>
        </div>
        ...
        <div class="space-y-6">
            ...
            <div class="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
                ...
                <div>
                    ...
                    <select name="status" class="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-emerald-500 focus:border-emerald-500">
                        <option value="Dipublikasi" <?= $data['artikel']['status'] === 'Dipublikasi' ? 'selected' : ''; ?>>Dipublikasi Langsung</option>
                        <option value="Draft" <?= $data['artikel']['status'] === 'Draft' ? 'selected' : ''; ?>>Simpan Sebagai Draft</option>
                    </select>
                </div>

                <div class="pt-2 border-t border-slate-100">
                    <label class="flex items-center gap-3 cursor-pointer">
                        <input type="checkbox" name="is_featured" value="1" <?= $data['artikel']['is_featured'] ? 'checked' : ''; ?> class="w-4 h-4 text-emerald-600 rounded focus:ring-emerald-500 border-slate-300">
                        <div>
                ...
                <div>
                    ...
                    <select name="kategori_id" required class="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-emerald-500 focus:border-emerald-500">
                        ...
                        <?php foreach($data['kategori_list'] as $kat): ?>
                            <option value="<?= $kat['id']; ?>" <?= $data['artikel']['kategori_id'] == $kat['id'] ? 'selected' : ''; ?>>
                ...
                <div>
                    ...
                    <div class="max-h-36 overflow-y-auto p-3 border border-slate-200 rounded-lg space-y-2 bg-slate-50">
                        <?php 
                        $selectedTagIds = $data['artikel']['tag_ids'] ?? [];
                        ...
                        <?php else: ?>
                            <?php foreach($data['tag_list'] as $tag): ?>
                                <label class="flex items-center gap-2 text-sm text-slate-700 cursor-pointer hover:text-emerald-700">
                                    <input type="checkbox" name="tags[]" value="<?= $tag['id']; ?>" <?= in_array($tag['id'], $selectedTagIds) ? 'checked' : ''; ?> class="w-4 h-4 text-emerald-600 rounded border-slate-300">
            ...
            <div class="bg-white p-6 rounded-xl border border-slate-200 shadow-sm space-y-4">
                ...
                <div>
                    <label class="block text-sm font-medium text-slate-700 mb-1">Ganti Berkas Gambar</label>
                ...
                <div>
                    <input type="url" name="gambar_url" value="<?= htmlspecialchars($data['artikel']['gambar_sampul'] ?? ''); ?>" placeholder="https://domain.com/gambar.jpg" class="w-full px-3 py-2 text-sm border border-slate-300 rounded-lg focus:ring-emerald-500 focus:border-emerald-500">
                </div>

                <!-- Preview Area -->
                <div id="coverPreviewContainer" class="<?= !empty($data['artikel']['gambar_sampul']) ? '' : 'hidden'; ?>">
                    <p class="text-xs font-semibold text-slate-500 mb-1">Pratinjau Sampul Saat Ini:</p>
                    <img id="coverPreview" src="<?= htmlspecialchars($data['artikel']['gambar_sampul'] ?? '#'); ?>" class="w-full h-40 object-cover rounded-lg border border-slate-200">
                </div>
            </div>
